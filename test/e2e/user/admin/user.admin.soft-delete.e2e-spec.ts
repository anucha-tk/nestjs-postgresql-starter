...
describe("user soft-delete e2e", () => {
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).delete(
        `${USER_SOFT_DELETE_URL}/${faker.number.int({ min: 1000, max: 2000 })}`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_SOFT_DELETE_URL}/${faker.number.int({ min: 1000, max: 2000 })}`)
  ...
  describe("guard", () => {
    ...
    it("should return 404 when delete your self", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_SOFT_DELETE_URL}/${admin.id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
    ...
    it("should return 404 when user has deleted", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_SOFT_DELETE_URL}/${userDelete.id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
