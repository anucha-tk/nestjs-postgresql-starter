...
describe("user blocked e2e", () => {
  const USER_BLOCKED_URL = "/admin/user/update";
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).patch(
        `${USER_BLOCKED_URL}/${faker.number.int({ min: 1000, max: 2000 })}/blocked`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_BLOCKED_URL}/${faker.number.int({ min: 1000, max: 2000 })}/blocked`)
  ...
  describe("guard", () => {
    ...
    it("should return 404 when your self", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_BLOCKED_URL}/${admin.id}/blocked`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
    ...
    it("should return 404 when user already blocked", async () => {
  ...
  describe("active response", () => {
    it("should return 200 when blocked user successful", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_BLOCKED_URL}/${user.id}/blocked`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
      ...
      const userBlocked = await userService.findOneById(user.id);

      expect(userBlocked.id).toBe(user.id);
      expect(userBlocked.blocked).toBeTruthy();
    });
