...
describe("user inactive e2e", () => {
  const USER_INACTIVE_URL = "/admin/user/update";
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).patch(
        `${USER_INACTIVE_URL}/${faker.number.int({ min: 1000, max: 2000 })}/inactive`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_INACTIVE_URL}/${faker.number.int({ min: 1000, max: 2000 })}/inactive`)
  ...
  describe("guard", () => {
    ...
    it("should return 404 when your self", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_INACTIVE_URL}/${admin.id}/inactive`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
    ...
    it("should return 400 when user is blocked", async () => {
      jest.spyOn(userService, "joinWithRole").mockResolvedValue({ ...user, blocked: true });
      const { status, body } = await request(app.getHttpServer())
        .patch(`${USER_INACTIVE_URL}/${user.id}/inactive`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
  ...
  describe("inactive response", () => {
    it("should return 200 when inactive user successful", async () => {
      jest.spyOn(userService, "joinWithRole").mockResolvedValue({
        ...user,
        isActive: true,
      });
