...
describe("user delete e2e", () => {
  const USER_DELETE_URL = "/admin/user/delete";
  ...
  beforeAll(async () => {
    ...
    userDeleted = await userFaker.createUser({ deleted: true });
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).delete(
        `${USER_DELETE_URL}/${faker.number.int({ min: 1000, max: 2000 })}`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_DELETE_URL}/${faker.number.int({ min: 1000, max: 2000 })}`)
  ...
  describe("guard", () => {
    ...
    it("should return 404 when user not have deletedAt", async () => {
      const { status, body } = await request(app.getHttpServer())
  ...
  describe("delete response", () => {
    it("should return 200 when user have deletedAt delete successful", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_DELETE_URL}/${userDeleted.id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);

      expect(status).toBe(200);
      expect(body.data).toEqual({ id: userDeleted.id });
    });
    it("should return 200 when softDelete and then delete user", async () => {
      const softDeleteRes = await request(app.getHttpServer())
        .delete(`${USER_SOFT_DELETE_URL}/${user.id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
      ...
      expect(status).toBe(200);
      expect(body.data).toEqual({ id: user.id });
    });
    it("should return 404 when delete user again", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${USER_DELETE_URL}/${user.id}`)
        .set("x-api-key", xApiKey)
        .set("authorization", `Bearer ${adminAccessToken}`);
