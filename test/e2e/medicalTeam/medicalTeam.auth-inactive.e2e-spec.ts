...
describe("medicalTeam auth inactive e2e", () => {
  const MEDICALTEAM_INACTIVE_URL = "/auth/medicalTeam/update";
  ...
  let medicalTeamInActiveId: number;
  ...
  beforeAll(async () => {
    ...
    medicalTeamInActiveId = await medicalTeamFaker
      .createMedicalTeam({ isActive: false })
      .then((e) => e.id);
  ...
  describe("Auth", () => {
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${MEDICALTEAM_INACTIVE_URL}/1/inactive`)
          .send();
    ...
    describe("guard", () => {
      ...
      it("should return 400 when medicalTeam already inactive", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${MEDICALTEAM_INACTIVE_URL}/${medicalTeamInActiveId}/inactive`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${userAccessToken}`);

        expect(status).toBe(400);
        expect(body.statusCode).toBe(
          ENUM_MEDICALTEAM_STATUS_CODE_ERROR.MEDICALTEAM_IS_ACTIVE_ERROR,
        );
      });
    });
  });
  ...
  describe("response", () => {
    it("should return 200 when inactive success", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${MEDICALTEAM_INACTIVE_URL}/${medicalTeamId}/inactive`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
