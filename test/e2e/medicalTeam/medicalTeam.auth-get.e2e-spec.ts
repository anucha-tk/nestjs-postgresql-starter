...
describe("medicalTeam auth get e2e", () => {
  const MEDICALTEAM_GET_URL = "/auth/medicalTeam/get";
  ...
  let medicalTeamWithPhonesId: number;
  ...
  beforeAll(async () => {
    ...
    medicalTeamWithPhonesId = await medicalTeamFaker
      .createMedicalTeam({ phones: true })
      .then((e) => e.id);
  ...
  describe("Auth", () => {
    ...
    describe("response", () => {
      it("should return medicalTeam", async () => {
        const { status, body } = await request(app.getHttpServer())
          .get(`${MEDICALTEAM_GET_URL}/${medicalTeamId}`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${userAccessToken}`);
        ...
        expect(body.data.phones).toHaveLength(0);
      });
      it("should return medicalTeam relative with phone", async () => {
        const { status, body } = await request(app.getHttpServer())
          .get(`${MEDICALTEAM_GET_URL}/${medicalTeamWithPhonesId}`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${userAccessToken}`);
        ...
        expect(body._metadata).toBeDefined();
        expect(body.data.id).toBeDefined();
        expect(body.data.phones).toHaveLength(1);
      });
    });
  });
});
