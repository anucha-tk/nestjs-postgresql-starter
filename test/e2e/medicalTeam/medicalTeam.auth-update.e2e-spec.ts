...
describe("medicalTeam auth active e2e", () => {
  const MEDICALTEAM_UPDATE_URL = "/auth/medicalTeam/update";
  ...
  describe("validation body", () => {
    it("should return 422 when empty body", async () => {
      const { status, body } = await request(app.getHttpServer())
        .put(`${MEDICALTEAM_UPDATE_URL}/${medicalTeamId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`)
        .send();
  ...
  describe("response", () => {
    it("should return 200 when update success", async () => {
      const dto = {
