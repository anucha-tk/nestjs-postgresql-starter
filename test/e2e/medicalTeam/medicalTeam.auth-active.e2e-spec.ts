...
describe("medicalTeam auth active e2e", () => {
  const MEDICALTEAM_ACTIVE_URL = "/auth/medicalTeam/update";
  ...
  describe("Auth", () => {
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${MEDICALTEAM_ACTIVE_URL}/1/active`)
          .send();
    ...
    describe("guard", () => {
      ...
      it("should return 400 when medicalTeam already active", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${MEDICALTEAM_ACTIVE_URL}/${medicalTeamId}/active`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${userAccessToken}`);
  ...
  describe("response", () => {
    it("should return 200 when active success", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${MEDICALTEAM_ACTIVE_URL}/${medicalTeamInActiveId}/active`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
