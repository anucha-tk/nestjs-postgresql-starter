...
describe("phone auth active e2e", () => {
  const PHONE_ACTIVE_URL = "/auth/phone/update";
    ...
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${PHONE_ACTIVE_URL}/123/active`)
          .send();
  ...
  describe("guard", () => {
    ...
    it("should return 400 phone already active", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${PHONE_ACTIVE_URL}/${phoneId}/active`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
  ...
  describe("response", () => {
    it("should return 200 and void when update phone active success", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${PHONE_ACTIVE_URL}/${phoneIdInActive}/active`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
