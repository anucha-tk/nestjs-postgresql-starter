...
describe("phone auth inactive e2e", () => {
  const PHONE_INACTIVE_URL = "/auth/phone/update";
  ...
  beforeAll(async () => {
    ...
    phoneIdInActive = await phoneFaker.createPhoneCustomer({ isActive: false }).then((e) => e.id);
    ...
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${PHONE_INACTIVE_URL}/123/inactive`)
          .send();
  ...
  describe("guard", () => {
    it("should return 404 phone not found", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${PHONE_INACTIVE_URL}/123/inactive`)
        .set("x-api-key", xApiKey)
    ...
    it("should return 400 phone already inactive", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${PHONE_INACTIVE_URL}/${phoneIdInActive}/inactive`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);

      expect(status).toBe(400);
      expect(body.statusCode).toBe(ENUM_PHONE_STATUS_CODE_ERROR.PHONE_IS_ACTIVE_ERROR);
    });
  });

  describe("response", () => {
    it("should return 200 and void when update phone inactive success", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${PHONE_INACTIVE_URL}/${phoneId}/inactive`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
