...
describe("phone auth get e2e", () => {
  const PHONE_GET_URL = "/auth/phone/get";
  ...
  describe("response", () => {
    it("should return 200 when get phone success", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${PHONE_GET_URL}/${phoneId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      ...
      expect(body.data.phoneNumber).toBeDefined();
