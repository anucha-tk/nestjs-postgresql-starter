...
describe("role inActive e2e", () => {
  ...
  let rolePublicInActive: RoleEntity;
  ...
  beforeAll(async () => {
    ...
    rolePublicInActive = await roleFaker.createRoleUser({ isActive: false });
  ...
  describe(`PATCH ${UPDATE_URL}/role/inactive`, () => {
    describe("x-api-key", () => {
      it("should return 401 when not send x-api-key", async () => {
        const { status, body } = await request(app.getHttpServer()).patch(
          `${UPDATE_URL}/${rolePublic.id}/inactive`,
        );
    ...
    describe("auth", () => {
      it("should return 401 when not auth", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${rolePublic.id}/inactive`)
    ...
    describe("get guard", () => {
      it("should return 404 when not not found role", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${faker.number.int({ min: 1000, max: 2000 })}/inactive`)
          .set("x-api-key", xApiKey)
      ...
      it("should return 400 when role inactive yet", async () => {
        const { status, body } = await request(app.getHttpServer())
          .patch(`${UPDATE_URL}/${rolePublicInActive.id}/inactive`)
          .set("x-api-key", xApiKey)
          .set("Authorization", `Bearer ${adminAccessToken}`);
