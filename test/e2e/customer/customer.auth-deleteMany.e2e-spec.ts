...
describe("customer auth controller deleteMany e2e", () => {
  const CUSTOMER_DELETE_URL = "/auth/customer/deleteMany";
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).delete(CUSTOMER_DELETE_URL);
  ...
  describe("response", () => {
    it("should return 200 and id array response", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(CUSTOMER_DELETE_URL)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`)
      ...
      expect(body.data.ids).toBeDefined();
    });
  });
});
