...
describe("customer auth controller recover e2e", () => {
  const CUSTOMER_RECOVER_URL = "/auth/customer/recover";
  ...
  beforeAll(async () => {
    ...
    customerDeletedId = await customerFaker.create({ deleted: true }).then((e) => e.id);
  ...
  describe("guards", () => {
    it("should return 404 when not found customer", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${CUSTOMER_RECOVER_URL}/1`)
        .set("x-api-key", xApiKey)
    ...
    it("should return 404 when customer not deleted", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${CUSTOMER_RECOVER_URL}/${customerId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
  ...
  describe("response", () => {
    it("should return 200 and id response", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${CUSTOMER_RECOVER_URL}/${customerDeletedId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
