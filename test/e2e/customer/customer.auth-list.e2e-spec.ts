...
describe("customer auth controller list e2e", () => {
  const CUSTOMER_LIST_URL = "/auth/customer/list";
  ...
  beforeAll(async () => {
    ...
    // mock customers
    await Promise.all([
      customerFaker.create({ isActive: false }),
      customerFaker.create({}),
      customerFaker.create({}),
      customerFaker.create({}),
    ]);
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).get(CUSTOMER_LIST_URL);
  ...
  describe("response", () => {
    it("should return 200 and 5 customer", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(CUSTOMER_LIST_URL)
        .set("x-api-key", xApiKey)
    ...
    it("should return 200 and 1 customer when query isActive false", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${CUSTOMER_LIST_URL}?isActive=false`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      expect(status).toBe(200);
    ...
    it("should return 200 and 1 customer when query id", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${CUSTOMER_LIST_URL}?ids=${customerId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      expect(status).toBe(200);
    ...
    it("should return 200 when query enum", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${CUSTOMER_LIST_URL}?type=UNKNOWN`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      expect(status).toBe(200);
    ...
    it("should return 200 when query relation", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${CUSTOMER_LIST_URL}?relations=address,phones`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      expect(status).toBe(200);
    ...
    it("should return 200 when query search", async () => {
      const { status, body } = await request(app.getHttpServer())
        .get(`${CUSTOMER_LIST_URL}?search=Avon`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
      expect(status).toBe(200);
