...
describe("customer auth controller update address e2e", () => {
  const CUSTOMER_UPDATE_URL = "/auth/customer/update";
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer())
        .put(`${CUSTOMER_UPDATE_URL}/1/address`)
        .send();
  ...
  describe("guard", () => {
    ...
    it("should return 422 when send empty body", async () => {
      const { status, body } = await request(app.getHttpServer())
        .put(`${CUSTOMER_UPDATE_URL}/${customerId}/address`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`)
        .send({});
  ...
  describe("response", () => {
    it("should return 200 when update address success", async () => {
      const dto = {
      ...
      expect(body.data.address.street).toBe(dto.street);
      expect(body.data.address.subDistrict).toBe(dto.subDistrict);
      expect(body.data.address.district).toBe(dto.district);
      expect(body.data.address.province).toBe(dto.province);
      expect(body.data.address.postalCode).toBe(dto.postalCode);
    });
  });
});
