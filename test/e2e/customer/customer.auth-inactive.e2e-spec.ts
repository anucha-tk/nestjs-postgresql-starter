...
describe("customer auth controller inactive e2e", () => {
  const CUSTOMER_INACTIVE_URL = "/auth/customer/update";
  ...
  beforeAll(async () => {
    ...
    customerInActiveId = await customerFaker.create({ isActive: false }).then((e) => e.id);
  ...
  describe("x-api-key", () => {
    it("should return 401 when not send x-api-key", async () => {
      const { status, body } = await request(app.getHttpServer()).patch(
        `${CUSTOMER_INACTIVE_URL}/1/inactive`,
      );
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_INACTIVE_URL}/1/inactive`)
  ...
  describe("guards", () => {
    ...
    it("should return 400 when customer already inactive", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_INACTIVE_URL}/${customerInActiveId}/inactive`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);

      expect(status).toBe(400);
      expect(body.statusCode).toBe(ENUM_CUSTOMER_STATUS_CODE_ERROR.CUSTOMER_IS_ACTIVE_ERROR);
    });
  });

  describe("response", () => {
    it("should return id and inactive false", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_INACTIVE_URL}/${customerId}/inactive`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
