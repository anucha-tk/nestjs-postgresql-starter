...
describe("customer auth controller active e2e", () => {
  const CUSTOMER_ACTIVE_URL = "/auth/customer/update";
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_ACTIVE_URL}/1/active`)
  ...
  describe("guards", () => {
    ...
    it("should return 400 when customer already active", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_ACTIVE_URL}/${customerId}/active`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
  ...
  describe("response", () => {
    it("should return id and isActive true", async () => {
      const { status, body } = await request(app.getHttpServer())
        .patch(`${CUSTOMER_ACTIVE_URL}/${customerInActiveId}/active`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
