...
describe("customer auth controller softRemove e2e", () => {
  const CUSTOMER_SOFT_REMOVE_URL = "/auth/customer/soft-remove";
  ...
  describe("auth", () => {
    it("should return 401 when not auth", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${CUSTOMER_SOFT_REMOVE_URL}/1`)
  ...
  describe("response", () => {
    it("should return 200 and id response", async () => {
      const { status, body } = await request(app.getHttpServer())
        .delete(`${CUSTOMER_SOFT_REMOVE_URL}/${customerId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`);
