...
describe("customer auth controller update e2e", () => {
  ...
  describe("guard", () => {
    ...
    it("should return 422 when send empty body", async () => {
      const { status, body } = await request(app.getHttpServer())
        .put(`${CUSTOMER_UPDATE_URL}/${customerId}`)
        .set("x-api-key", xApiKey)
        .set("Authorization", `Bearer ${userAccessToken}`)
        .send({});
  ...
  describe("response", () => {
    it("should return 200 when get success", async () => {
      ...
      expect(body.data.thaiName).toBe(dto.thaiName);
      expect(body.data.englishName).toBe(dto.englishName);
      expect(body.data.tax).toBe(dto.tax);
      expect(body.data.type).toBe(dto.type);
    });
  });
});
