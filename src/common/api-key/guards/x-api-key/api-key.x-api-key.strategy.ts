...
export class ApiKeyXApiKeyStrategy extends PassportStrategy(Strategy, "api-key") {
  ...
  async validate(
    apiKey: string,
    verified: (error: Error, user?: ApiKeyEntity, info?: string | number) => Promise<void>,
    req: IRequestApp,
  ): Promise<void> {
    const {
    ...
    if (xApiKey.length !== 2) {
      verified(new Error(`${API_KEY_NEEDED_ERROR}`), null, null);
      return;
    }
    ...
    if (!authApi) {
      verified(new Error(`${API_KEY_NOT_FOUND_ERROR}`), null, null);
      return;
    } else if (!authApi.isActive) {
      verified(new Error(`${API_KEY_IS_ACTIVE_ERROR}`), null, null);
      return;
    } else if (authApi.startDate && authApi.endDate) {
      if (today < authApi.startDate) {
        verified(new Error(`${API_KEY_NOT_ACTIVE_YET_ERROR}`), null, null);
      } else if (today > authApi.endDate) {
        verified(new Error(`${API_KEY_EXPIRED_ERROR}`), null, null);
      }
    }
    ...
    if (!validateApiKey) {
      verified(new Error(`${API_KEY_INVALID_ERROR}`), null, null);
      return;
    }
    req.apiKey = {
      id: authApi.id,
