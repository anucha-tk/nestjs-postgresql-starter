...
/**
 * FileValidationPipe.
 * validation class dto
 * @description use only excel
 * @description use after FileExtractPipe
 */
@Injectable()
export class FileValidationPipe<T> implements PipeTransform {
  ...
  async transform(
    ...
    if (Array.isArray(value)) {
      ...
      for (const val of value) {
        ...
        const classTransform: T[] = await this.transformExtract(this.dto, val.extract);

        await this.validateExtract(classTransform, val.filename);

        const classTransformMerge: IFileExtract<T> = await this.transformMerge(val, classTransform);
    ...
    const classTransform: T[] = await this.transformExtract(this.dto, file.extract);
  ...
  async transformMerge(value: IFileExtract, classTransform: T[]): Promise<IFileExtract<T>> {
    return {
      ...value,
      dto: classTransform,
    };
  ...
  async validate(value: IFileExtract): Promise<void> {
    if (!Object.values(ENUM_FILE_EXCEL_MIME).find((val) => val === value.mimetype.toLowerCase())) {
  ...
  async validateExtract(classTransform: T[], filename: string): Promise<void> {
    ...
    for (const [index, clsTransform] of classTransform.entries()) {
      const validator: ValidationError[] = await validate(clsTransform as Record<string, any>);
