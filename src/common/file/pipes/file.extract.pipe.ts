 ...
 * FileExtractPipe.
 * extract all excel
 * @description use excel only
 * @throws 422 if not excel type
 */
@Injectable()
export class FileExtractPipe implements PipeTransform {
  constructor(private readonly helperFileService: HelperFileService) {}

  async transform(value: IFile | IFile[]): Promise<IFileExtract | IFileExtract[]> {
    if (!value) {
      return;
    }
  ...
  async validate(mimetype: string): Promise<void> {
    if (!Object.values(ENUM_FILE_EXCEL_MIME).find((val) => val === mimetype.toLowerCase())) {
