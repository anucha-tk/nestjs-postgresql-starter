    ...
    if (context.getType() === "http") {
      ...
      return next.handle().pipe(
        tap(async (response: Promise<Record<string, any>>) => {
          ...
          const apiKeyEntity: ApiKeyEntity = apiKey
            ? await this.apiKeyService.findOneById(apiKey.id)
            : null;
          ...
          await this.loggerService.raw({
            ...
            apiKey: apiKeyEntity,
            // WARN: we can't check user instanceof UserEntity, but check has email instead
            user: user?.email ? user?.id : undefined,
            requestId: __id,
            method: method as ENUM_REQUEST_METHOD,
            // WARN: we can't check user instanceof UserEntity, but check has role instead
